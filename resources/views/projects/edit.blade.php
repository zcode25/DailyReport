        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                  <form method="post" action="{{ route('project.update', ['project' => $project->projectId]) }}">
                    @csrf
                    <h2 class="text-2xl font-semibold mb-6">Update Project Form</h2>

                    <div class="mb-4">
                      <label class="block text-gray-700 mb-2" for="projectName">Project Name</label>
                      <input class="input input-bordered w-full bg-white @error('projectName') input-error @enderror" type="text" id="projectName" name="projectName" value="{{ old('projectName', $project->projectName) }}">
                      @error('projectName')
                    ...
                    <div class="mb-4">
                        ...
                        <input class="input input-bordered w-full bg-white @error('projectAddress') input-error @enderror" type="text" id="address" name="address" value="{{ old('address', $project->address) }}">
                        @error('address')
                          <div class="text-rose-500 text-sm">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                      ...
                      <textarea class="textarea textarea-bordered w-full bg-white @error('projectDesc') input-error @enderror" id="projectDesc" name="projectDesc" rows="4">{{ old('projectDesc', $project->projectDesc) }}</textarea>
                      @error('projectDesc')
                        <div class="text-rose-500 text-sm">{{ $message }}</div>
                      @enderror
                    </div>
